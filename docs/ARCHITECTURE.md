...
PayTrace uses a feature-oriented Spring backend and a separate Next.js operator UI. H2 file mode retains local evidence. The event store preserves `occurredAt` and `receivedAt` to separate financial order from arrival order.
